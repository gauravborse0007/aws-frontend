...
const Signup = () => {
  ...
  const handleSignup = async (e) => {
    ...
    try {
      ...
      // localStorage is a browser storage. Stores the data  In key–value form
      // here "token" is key and "res.data.token" is the value, here value comes from backend
      localStorage.setItem("token", res.data.token); 
      localStorage.setItem("userId", res.data.userId);

      setCurrentUser(res.data.userId);// here we are setting loggedIn user's userId in auth context so user should logged in session. Refer to line 19
      setLoading(false);
